...
namespace gpr
{
  ...
  struct comment_data
  {
    ...
    comment_data(const char p_left_delim,
                 ...
                 const std::string &p_comment_text) : left_delim(p_left_delim),
  ...
  public:
    ...
    chunk(const char p_left_delim,
          const char p_right_delim,
          const std::string &p_comment_text) : chunk_tp(CHUNK_TYPE_COMMENT),
                                               cd(p_left_delim, p_right_delim, p_comment_text)
    {
    }

    chunk(const char p_wd,
          const addr p_adr) : chunk_tp(CHUNK_TYPE_WORD_ADDRESS),
                              cd('(', ')', ""),
                              wad(p_wd, p_adr)
    {
    }
  ...
  public:
    block(const int p_line_no,
          ...
          const std::vector<chunk> p_chunks) : has_line_no(true),
                                               line_no(p_line_no),
    ...
    block(const bool p_slashed_out,
          const std::vector<chunk> p_chunks) : has_line_no(false),
                                               line_no(-1),
                                               slashed_out(p_slashed_out),
                                               chunks(p_chunks)
    {
    ...
    block(const block &other) : has_line_no(other.has_line_no),
